...
if not os.path.exists('hearst_ppmi_filtered.csv'):
    ...
    with NamedTemporaryFile(suffix='.txt.gz') as tfile:
        url = 'https://github.com/facebookresearch/hypernymysuite/raw/master/hearst_counts.txt.gz'
    ...
    df['ppmi'] = np.clip(np.log(N) + np.log(df['count']) - np.log(df['pw']) - np.log(df['pc']), 0, 1e12)
    df = df.rename(columns={'ppmi': 'weight'})
    df[df['weight'] > 0][['id1', 'id2', 'weight']].to_csv('hearst_ppmi_filtered.csv', index=False)
